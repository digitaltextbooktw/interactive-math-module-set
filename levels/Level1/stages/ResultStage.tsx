...
export default function ResultStage({ score, wrong, onRestart, onContinue }: Props) {
  ...
  const glowColor = grade === 'gold' ? 'rgba(251,191,36,0.5)' : grade === 'silver' ? 'rgba(148,163,175,0.4)' : 'rgba(180,127,9,0.35)';
  ...
  return (
    <div style={{
      ...
      {/* Shield with color-changing triangle */}
      <div style={{
        ...
        <svg viewBox="0 0 100 100" width="150" height="150">
          ...
          <polygon points="50,28 32,58 68,58"
            fill={triColor} opacity="0.35" filter="url(#triGlow)"
            style={{
              opacity: shieldVisible ? 0.35 : 0,
              transition: 'opacity 1.2s ease-out 0.8s',
            }}
          />

          {/* Main triangle — color based on grade */}
          <polygon points="50,28 32,58 68,58"
            fill={triColor}
            style={{
              opacity: shieldVisible ? 1 : 0,
              transform: shieldVisible ? 'translateY(0)' : 'translateY(30px)',
              transition: 'all 0.8s ease-out 0.3s',
              transformOrigin: '50px 45px',
            }}
          />

          {/* Shimmer overlay on triangle */}
          <polygon points="50,28 32,58 68,58"
            fill="url(#shimmerGrad)"
            style={{
              opacity: shieldVisible ? 0.6 : 0,
      ...
      <div style={{
        ...
        transform: scoreVisible ? 'translateY(0)' : 'translateY(15px)',
        transition: 'all 0.5s ease-out',
        fontSize: '1.1rem', color: '#3d5a80', fontWeight: 900,
      }}>
        答對 {score}/4 題
      </div>
      ...
      <div style={{
        ...
        transform: dialogVisible ? 'translateY(0)' : 'translateY(15px)',
        transition: 'all 0.5s ease-out',
        display: 'flex', alignItems: 'center', gap: 14,
        background: 'rgba(255,255,255,0.95)', border: '1px solid #E5E7EB',
        borderRadius: 16, padding: '14px 20px', marginTop: 8,
        ...
        <div style={{ textAlign: 'left' }}>
          <div style={{ fontWeight: 900, fontSize: 13, color: '#293241', marginBottom: 2 }}>老角</div>
          <div style={{ fontSize: 15, color: '#293241', lineHeight: 1.5 }}>{dialog.text}</div>
        </div>
      </div>

      {/* Buttons: restart (icon) + continue (icon) */}
      <div style={{
