...
// Label position outside triangle (away from centroid)
function outerLabel(vertex: Point, centroid: Point, dist: number) {
...
export default function ExploreStage({ onComplete }: { onComplete: () => void }) {
  ...
  useEffect(() => {
    ...
    const check = () => {
      if (dragCount >= 3 || Date.now() - startTime.current > 10000) setShowNudge(true);
    };
    check();
  ...
  useEffect(() => {
    if (showNudge && dragCount >= 5 && !showButton) setShowButton(true);
  ...
  const lblA = outerLabel(A, centroid, 36);
  const lblB = outerLabel(B, centroid, 36);
  const lblC = outerLabel(C, centroid, 36);
    ...
    <div style={{ display: 'flex', height: '100%', padding: 'clamp(8px, 1.5vmin, 12px) clamp(8px, 2vmin, 16px)', gap: 'clamp(8px, 1.5vmin, 12px)', position: 'relative' }}>
      ...
      <div style={{ flex: 1, background: 'white', borderRadius: 16, border: '1px solid #E5E7EB', overflow: 'hidden', position: 'relative', minWidth: 0 }}>
        ...
        >
          ...
          <path d={drawSector(A, C, B, 30)} fill="#98c1d9" fillOpacity="0.35" stroke="#3d5a80" strokeWidth="1" />
          {/* Interior angle sectors: B (blue) */}
          <path d={drawSector(B, A, C, 30)} fill="#98c1d9" fillOpacity="0.35" stroke="#3d5a80" strokeWidth="1" />
          {/* Interior angle C (light) */}
          <path d={drawSector(C, B, A, 32)} fill="#98c1d9" fillOpacity="0.12" stroke="#3d5a80" strokeWidth="0.8" />

          {/* Exterior angle at C (orange) — between extension line D and edge CB */}
          <path d={drawSector(C, D, B, 32)} fill="#ee6c4d" fillOpacity="0.35" stroke="#ee6c4d" strokeWidth="1.5" />

          {/* Angle degree labels inside sectors */}
          {/* A */}
          {(() => {
            const mid = outerLabel(A, centroid, -44); // toward centroid, away from arc edge
            return <text x={mid.x} y={mid.y + 5} textAnchor="middle" fontSize="13" fontWeight="900" fill="#3d5a80">{angles.A}°</text>;
          })()}
          {/* B */}
          {(() => {
            const mid = outerLabel(B, centroid, -44);
            return <text x={mid.x} y={mid.y + 5} textAnchor="middle" fontSize="13" fontWeight="900" fill="#3d5a80">{angles.B}°</text>;
          })()}
          {/* Exterior angle label */}
          {(() => {
            const midExt = { x: (C.x + D.x) / 2, y: (C.y + D.y) / 2 };
            const offX = B.x > C.x ? 25 : -25;
            return <text x={midExt.x + offX} y={midExt.y - 10} textAnchor="middle" fontSize="16" fontWeight="900" fill="#ee6c4d">{angles.ext}°</text>;
          })()}

          {/* Vertex labels — OUTSIDE the triangle, no ∠ symbol */}
          <text x={lblA.x} y={lblA.y + 5} textAnchor="middle" fontSize="14" fontWeight="900" fill="#3d5a80">A</text>
          <text x={lblB.x} y={lblB.y + 5} textAnchor="middle" fontSize="14" fontWeight="900" fill="#3d5a80">B</text>
          <text x={lblC.x} y={lblC.y + 5} textAnchor="middle" fontSize="14" fontWeight="900" fill="#ee6c4d">C</text>
          ...
          {pts.map((p, i) => (
            ...
            >
              <circle cx={p.x} cy={p.y} r="16" fill="white"
                stroke={draggingIdx === i ? '#ee6c4d' : '#3d5a80'} strokeWidth="3" />
      ...
      {/* Right panel */}
      <div style={{ width: 'clamp(120px, 20vmin, 170px)', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 8 }}>
        <div style={{ background: 'white', borderRadius: 14, border: '1px solid #E5E7EB', padding: 'clamp(10px, 1.8vmin, 14px) clamp(10px, 2vmin, 16px)' }}>
          <div style={{ fontSize: 'clamp(12px, 1.8vmin, 14px)', fontWeight: 700, color: '#3d5a80', marginBottom: 4 }}>A = {angles.A}°</div>
          <div style={{ fontSize: 'clamp(12px, 1.8vmin, 14px)', fontWeight: 700, color: '#3d5a80', marginBottom: 8 }}>B = {angles.B}°</div>
          <div style={{ borderTop: '2px solid #F1F5F9', paddingTop: 8, marginBottom: 8 }}>
            <div style={{
              fontSize: 'clamp(14px, 2vmin, 16px)', fontWeight: 900,
          ...
          <div style={{ borderTop: '2px solid #F1F5F9', paddingTop: 8 }}>
            <div style={{
              fontSize: 'clamp(15px, 2.2vmin, 18px)', fontWeight: 900,
        ...
        {showButton && !showModal && (
          <button onClick={() => { setShowButton(false); setShowModal(true); }} style={{
            ...
            padding: '10px 16px', fontWeight: 900, fontSize: 14, cursor: 'pointer',
      ...
      {showModal && (
        <div style={{
          ...
          <div style={{
            ...
            maxWidth: 360, width: '85%', boxShadow: '0 20px 40px rgba(0,0,0,0.15)',
          }}>
            {modalAnswer === discoveryCorrectIndex ? (
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 'clamp(22px, 4vmin, 28px)', fontWeight: 900, color: '#10B981', marginBottom: 8 }}>沒錯！</div>
                <div style={{ fontSize: 'clamp(14px, 2vmin, 16px)', color: '#293241', fontWeight: 700 }}>外角 = A + B</div>
              </div>
            ) : (
              <>
                <div style={{ fontSize: 'clamp(15px, 2.5vmin, 18px)', fontWeight: 900, color: '#293241', marginBottom: 'clamp(10px, 2vmin, 16px)', textAlign: 'center' }}>
                  {discoveryQuestion}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(8px, 1.2vmin, 10px)' }}>
                  {discoveryOptions.map((opt, i) => {
                    ...
                    return (
                      <button key={i} onClick={() => handleModalAnswer(i)}
                        ...
                        style={{
                          padding: 'clamp(8px, 1.5vmin, 12px) clamp(12px, 2vmin, 16px)',
                          border: `2px solid ${border}`, borderRadius: 10,
                          background: bg, fontWeight: 700, fontSize: 'clamp(13px, 2vmin, 15px)',
