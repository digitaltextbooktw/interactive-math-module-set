...
type Stage = 'intro' | 'guess' | 'explore' | 'reveal' | 'quiz' | 'result';
...
export default function Level2({ startStage, guessAnswer: initialGuess, onExit, onComplete }: Props) {
  const [stage, setStage] = useState<Stage>((startStage as Stage) || 'intro');
  ...
  const goTo = useCallback((next: Stage) => {
    setExitAnim(true);
    setTimeout(() => { setStage(next); setExitAnim(false); }, 300);
  }, []);
  ...
  return (
    <div style={{
      ...
      {onExit && (
        <button onClick={onExit} title="回到星球" style={{
      ...
      <div style={{
        ...
        {stage === 'intro' && (
          <div style={{ position: 'relative', width: '100%', height: '100%', background: '#0f1523' }}>
            <DialogEngine lines={introDialog} onComplete={() => goTo('guess')} />
          </div>
        )}
        {stage === 'guess' && <GuessStage onComplete={handleGuessComplete} />}
        {stage === 'explore' && <ExploreStage onComplete={() => goTo('reveal')} />}
        {stage === 'reveal' && <RevealStage onComplete={() => goTo('quiz')} />}
        {stage === 'quiz' && <QuizStage onComplete={handleQuizComplete} />}
        {stage === 'result' && (
          <ResultStage
            score={result.score} wrong={result.wrong}
            onRestart={() => goTo('quiz')}
