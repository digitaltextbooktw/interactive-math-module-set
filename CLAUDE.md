...
- **Build**: Vite (dev server port 3000, host 0.0.0.0)
- **Styling**: Tailwind CSS (CDN), inline styles, `index.css`（全域樣式）
- **Fonts**: 源泉圓體 GenSenRounded (主字體 `--font-main`, emfont CDN), Nunito (數字/英文 `--font-en` / `.font-en`), Noto Sans TC (備用)
- **Path alias**: `@` → 專案根目錄（vite.config.ts）
- **No state management library** — 使用 React useState/useEffect
- **Container**: Dockerfile (Node 20 Alpine build → Nginx Alpine serve, port 8080)
...
```bash
gcloud run deploy interactive-math-module-set --source . --region us-west1 --allow-unauthenticated --port 8080
```

- **Service URL**: https://interactive-math-module-set-1075853042812.us-west1.run.app
- **Project**: meta-territory-482002-j5
- **Region**: us-west1
...
index.css           # 全域樣式（動畫 keyframes、touch-action、字體等）
index.tsx           # React root mount
App.tsx             # 主應用：首頁章節列表、模組頁面、練習模式路由
types.ts            # 共用型別 (Point, Module, ModuleInfo, DialogLine, QuizQuestion)
vite.config.ts      # Vite 設定，含 path alias 與 GEMINI_API_KEY 環境變數注入
Dockerfile          # 多階段建置（Node 20 build → Nginx serve）
...
levels/                      # 各模組的練習關卡（含 stages/ 子資料夾）
  Level1/                    # 三角形的角（Explore → Guess → Reveal → Quiz → Result）
    ...
    data.ts, index.tsx
  Level2/                    # 外角關係
    stages/                  # ExploreStage, ExteriorAngleAnim, Level2Explore, GuessStage, RevealStage, QuizStage, ResultStage
    data.ts, index.tsx
  Level3/                    # 全等判定（SSS/SAS/全等判斷）
    stages/                  # ExploreStage, Level3Explore, SASExplore, JudgeExplore, RevealStage, QuizStage
    data.ts                  # 匯出 sssQuizQuestions, sasQuizQuestions, judgeQuizQuestions, Section 型別
    geometry.ts              # 幾何計算工具函式
...
Level3 包含三個子區塊（`Section` 型別）：
- **sss**: SSS 全等作圖探索（ExploreStage）— 尺規作圖動畫 → 拼圖拖曳 → 唯一性驗證
- **sas**: SAS 全等探索（SASExplore）— SAS 作圖端點、線條色彩層次
- **judge**: 全等判斷入口（JudgeExplore）— 綜合全等條件判斷

每個子區塊有獨立的 quiz 題組（`sssQuizQuestions` / `sasQuizQuestions` / `judgeQuizQuestions`）。

## Touch/Drag 事件處理模式

互動拖曳採用 **window 層級事件監聽**（非 SVG 元素層級），避免 iPad 觸控重複觸發：
- `onPointerDown` / `onTouchStart`：掛在可拖曳元素上，啟動拖曳
- `pointermove` / `touchmove`：掛在 `window`（useEffect 內），處理拖曳移動
- `pointerup` / `touchend`：掛在 `window`，結束拖曳
- `touchmove` 需 `{ passive: false }` 以允許 `preventDefault()` 阻止捲動
- SVG 元素設 `style={{ touchAction: 'none' }}` 防止瀏覽器預設觸控行為
- **不要**在 SVG 上同時掛 React 事件處理器 + window 監聯器，會造成 iPad 上重複觸發卡頓
...
- `levels/Level4參考/`（面積計算）、`levels/Level6參考/`（邊長關係）為已完成但尚未整合進 App.tsx 路由的關卡
- `@types/react` + `@types/react-dom` 為 devDependencies（React 19 不內建型別）
